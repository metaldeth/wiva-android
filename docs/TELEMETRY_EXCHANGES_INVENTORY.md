...
Источник семантики: **wiva_electron** `src/main/modules/telemetry/`. Транспорт: **legacy Android kiosk** `data/remote/telemetry/*` (адаптировано в `wiva-android` под `com.wiva.android`).

| Топик / `type` | Направление | Файл эталона (TS) | Планируемый / фактический Kotlin |
| --- | --- | --- | --- |
| `cellStoreRequestExport` | → сервер | `manager.ts` onConnect, `requestCellStoreExport` | `WivaTelemetryService.sendInitialExchange` |
| `machineInfo` | → запрос | `manager.ts` onConnect | `WivaTelemetryService.sendInitialExchange` |
| `machineInfo` | ← ответ | `exchanges/machineInfo/types/machineInfoResponse.ts`, `processMachineInfoResponse` | `WivaTelemetryService.handleMachineInfo` → `MachineRegistration` |
| `baseIngredientRequestExportTopic` | → запрос | `manager.ts` onConnect, `requestBaseIngredientExport` | `WivaTelemetryService.sendInitialExchange` |
...
| `statusSubscribeTopic` | → | `manager.ts` `sendSubscriptionRequest` (`body` = UUID строкой) | `WivaTelemetryService.sendStatusSubscribeTopic` (`clientId`, `type`, `body` строка) |
...
| `saleImportTopic` | → | `manager.ts` `sendSaleImportTopic` | `WivaTelemetryService.sendSaleImportTopic`, демо D4, триггер после Pax 4 |
...
| `authCodeRequestExport` | → / ← | `manager.ts` `sendAuthCodeRequest`, `exchanges/authCodeRequestExport/types/...` | `WivaTelemetryService.sendAuthCodeRequest` |

Регистрация REST: `registration.ts` → `TelemetryApiService.registerMachine` + `WivaTelemetryService.registerMachine` (modelName/machineName **WIVA**).
