...
- **D4:** Модели JSON `saleImportTopic` как в wiva (`WivaTelemetrySaleDtos`); отправка демо с сервис-экрана; при коде Pax **4** (оплата прошла) из `PaymentTerminalService` — автоматическая попытка отправить тот же демо-payload (связка «событие терминала → telemetry»). Юнит-тест на структуру payload.
