...
class SimpleTelemetryCoordinatorTest {
    ...
    fun `reserve and enroll persists serial and credential`() = runTest {
        ...
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody("""{"serialNumber":"WIVA-000010","reservationToken":"rt","expiresAt":"2026-07-17T00:00:00Z"}"""),
        )
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody("""{"machineId":"m-10","serialNumber":"WIVA-000010","wsProtocolUrl":"ws://localhost/ws"}"""),
        )
        // when
        ...
        // then
        assertEquals("WIVA-000010", reg.serialNumber)
    ...
    fun `enroll conflict surfaces SerialAlreadyBoundException`() = runTest {
        ...
        // when
        val result = coordinator.enrollMachine("WIVA-000001", rebind = false)
        // then
    ...
    fun `connect with legacy stored ws resolves derived wss url`() = runTest {
        ...
        configRepository.setJson(
            ...
            jsonEncoder.encodeToString(
                com.wiva.android.domain.model.MachineRegistration.serializer(),
