...
class MvpTelemetryApiClientTest {
    ...
    fun `reserve sends X-Enrollment-Key and parses response`() {
        ...
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody("""{"serialNumber":"WIVA-000007","reservationToken":"rt","expiresAt":"2026-07-17T00:00:00Z"}"""),
        )
        // when
        ...
        assertEquals("WIVA-000007", result.getOrThrow().serialNumber)
    }
    ...
    fun `enroll idempotent second call succeeds`() {
        // given
        val body =
            """{"machineId":"m1","serialNumber":"WIVA-000001","wsProtocolUrl":"ws://localhost/ws"}"""
    ...
    fun `rebind sends rebind true in JSON body`() {
        ...
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody("""{"machineId":"m1","serialNumber":"WIVA-000001"}"""),
        )
        val request =
            EnrollRequestDto(
                installationId = "inst",
