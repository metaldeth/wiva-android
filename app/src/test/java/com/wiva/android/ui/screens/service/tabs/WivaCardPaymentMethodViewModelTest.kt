...
class WivaCardPaymentMethodViewModelTest {
    ...
    fun initialLoad_whenRepositoryAQSI_uiReflectsAqsi() =
        runBlocking {
            ...
            val vm = WivaCardPaymentMethodViewModel(fake, FakeMockRepo(CardPaymentMockMode.Aqsi))
    ...
    fun selectPax_callsSetSelectedPax() =
        runBlocking {
            val fake = FakeRepo(CardPaymentMethod.Aqsi)
            val vm = WivaCardPaymentMethodViewModel(fake, FakeMockRepo())
            vm.selectPax()
