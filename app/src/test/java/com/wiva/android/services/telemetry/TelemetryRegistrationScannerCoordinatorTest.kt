    ...
    fun `scan event fills registration key and serial from json qr`() = runTest {
        ...
        flow.emit(
            BarcodeEvent.TelemetryRegistrationQr(
                """
                {"type":"WIVA_TELEMETRY_REGISTRATION","version":1,"registrationKey":"REG-0123456789AB","serialNumber":"WIVA-000004","apiUrl":"https://194.67.74.147"}
                """.trimIndent(),
            ),
        )
        advanceUntilIdle()
        ...
        assertEquals("WIVA-000004", event.serialNumber)
