...
class MvpTelemetryDtosTest {
    ...
    fun `reserve response uses exact field names`() {
        ...
        val raw =
            """
            {"serialNumber":"VIWA-000042","reservationToken":"rtok","expiresAt":"2026-07-17T12:00:00Z"}
            """.trimIndent()
        // when
        val dto = json.decodeFromString(ReserveSerialResponseDto.serializer(), raw)
        // then
        assertEquals("VIWA-000042", dto.serialNumber)
    ...
    fun `enroll request uses contract field names`() {
        ...
        val dto =
            EnrollRequestDto(
                ...
                app = EnrollAppDto("1.0", 1, "com.viwa.android"),
            )
        // when
        val raw = json.encodeToString(dto)
        // then
        ...
        assertTrue(raw.contains("\"package\":\"com.viwa.android\""))
    }
