...
import com.wiva.android.ui.components.keyboard.WivaAlphanumericKeyboard
import com.wiva.android.ui.components.keyboard.WivaNumericKeyboard
 ...
 * Предоставляется через [LocalServiceKeyboardHost] внутри [WivaServiceMenuContent].
 */
...
internal val LocalServiceKeyboardHost =
    compositionLocalOf<ServiceKeyboardHostController> {
        error("LocalServiceKeyboardHost: только внутри сервисного меню (WivaServiceMenuContent)")
    }
                    ...
                    WivaNumericKeyboard(
                        ...
                        onKey = { key ->
                            if (kind.showDecimalPoint || key != ".") {
                                val cur = s.getValue()
                                if (cur.length >= s.maxLength) return@WivaNumericKeyboard
                                val next = cur + key
